...
(async () => {
  ...
  // ---------- 6. 10th grade turning point ----------
  {
    ...
    const steps = [
      ...
      ["2", "ההבנה", "אני לא ״ילד רע״ כמו שכולם אומרים. זו תדמית, לא אני."],
  ...
  // ---------- 9. four cards ----------
  {
    ...
    const cards = [
      ...
      [ic.userFriends, "מי השפיע עליי", "קליין וברנש, המפקצ׳ והמג״ד שלי. אנשים שראו בי משהו לפני שראיתי אותו בעצמי."],
  ...
  // ---------- 13. inside the moment ----------
  {
    ...
    s.addText("(כאן משלימים בעל פה – לא כותבים על השקף)", T({ x: 6.6, y: 5.0, w: W - 0.9 - 6.6, h: 0.6, fontSize: 14, italic: true, color: C.mutedL }));
    pageNum(s, n, true);
    s.addNotes("הרגע של הפציעה. לעצור על השקף הזה. להשלים את המשפט בעל פה – מה באמת עבר לך בראש. זה השקף הכי חשוב בפרק.");
  }
  ...
  // ---------- 18. turning back ----------
  {
    const s = pres.addSlide(); n++;
    bg(s, C.ink);
    s.addText("עברתי תהליך ארוך", T({ x: 1.0, y: 1.6, w: W - 2.0, h: 1.0, fontSize: 40, bold: true, color: C.mutedL, align: "center" }));
    s.addText("עד שהבנתי כמה משמעותיים הקשרים בחיים שלי.", T({ x: 1.0, y: 2.7, w: W - 2.0, h: 1.0, fontSize: 40, bold: true, color: "FFFFFF", align: "center" }));
    s.addText("היום אני הרבה יותר פתוח.", T({ x: 1.0, y: 4.0, w: W - 2.0, h: 1.0, fontSize: 44, bold: true, color: C.sand, align: "center" }));
    photoSlot(s, 4.4, 5.2, 4.5, 1.9, "חנוך עם חברים / משפחה – היום", true, camD);
    pageNum(s, n, true);
    s.addNotes("מה היה הרגע שבו זה השתנה? מי היה שם? (לספר סיפור אחד קטן.)");
  }
  ...
  // ---------- 22. today ----------
  {
    ...
    const items = [
      ...
      [ic.target, "מטרות", "יעדים ברורים לשנה הקרובה – ולחמש הבאות."],
  ...
  // ---------- 25. final ----------
  {
    ...
    s.addText("חנוך  |  [טלפון / אינסטגרם / מייל להרצאות]", T({ x: 1.0, y: 6.1, w: W - 2.0, h: 0.5, fontSize: 14, color: "5A4A30", align: "center" }));
